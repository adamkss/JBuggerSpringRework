...
public class SingletonInit {
    ...
    private void init(){
       ...
       // admin.getNotifications().add(notification2);
        ...
        bugRepository.save(new Bug("Buggy Facebook","The facebook main page is quite buggy","1.0",null, LocalDate.now(), Severity.CRITICAL,dev1,Status.NEW,dev2));
        bugRepository.save(new Bug("Buggy Instagram","The instagram main page is very buggy","1.0",null, LocalDate.now(), Severity.CRITICAL,admin,Status.IN_PROGRESS,dev1));
        bugRepository.save(new Bug("Buggy Twitter","The twitter is buggy","2.0","2.0", LocalDate.now().minusDays(1), Severity.HIGH,admin,Status.FIXED,dev2));
        bugRepository.save(new Bug("Buggy Pinterest","Pinterest is really buggy","3.0",null, LocalDate.now().minusDays(10), Severity.LOW,admin,Status.IN_PROGRESS,dev1));
        bugRepository.save(new Bug("Buggy Uber","Uber doesn't work!","4.0",null, LocalDate.now().minusDays(25), Severity.HIGH,tester,Status.IN_PROGRESS,admin));
        bugRepository.save(new Bug("Bug in Autovit","Autovit doesn't show the cars...","1.0",null, LocalDate.now().minusDays(25), Severity.HIGH,tester,Status.NEW,admin));
        bugRepository.save(new Bug("Bug in WizzAir","WizzAir planes don't fly","2.0.1",null, LocalDate.now().minusDays(14), Severity.CRITICAL,dev1,Status.NEW,dev1));
    }
}
